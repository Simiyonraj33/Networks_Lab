...
void mac_to_bin(const char *mac_str, char *binary_str) 
{
...
int build_frames(Packet packets[], int total_packets, const char *src_mac, const char *dest_mac, Frame frames[]) 
{
    ...
    for (int p = 0; p < total_packets; p++) 
    {
        ...
        while (curr_bit < 32) 
        {
            ...
            for (int b = 0; b < actual_bits_in_frame; b++) 
                frames[f_idx].data[padding + b] = packets[p].data[curr_bit + b];
            
            frames[f_idx].data[16] = '\0';
            strcpy(frames[f_idx].trailer, "0000000000000000");
...
void display(int app_len, int trans_len, int net_len, int total_packets, int total_frames) 
{
    printf("Application Layer Data        = %3d bits\n", app_len);
    printf("    v  \n");
    printf("    v  (Encap Transport Header: +32 bits)\n");
    printf("    v  \n");
    printf("Transport Layer Data     = %3d bits\n", trans_len);
    printf("    v  \n");
    printf("    v  (Encap Network Header:   +64 bits)\n");
    printf("    v  \n");
    printf("Network Layer Data       = %3d bits\n", net_len);
    printf("    v  \n");
    printf("    v  \n");
    printf("Packets Generated             = %3d packets\n", total_packets);
    printf("    v  \n");
    printf("    v  \n");
    printf("Split into 16-bit sized Frames including MAC Headers/Trailers...\n");
...
int main() 
{
    ...
    printf("Transport Layer Data Bitstream:\n%s\n", trans_stream);
    ...
    printf("Packet Header : %s%s\n", src_ip_bin, dest_ip_bin);
    printf("Network Layer Data Bitstream:\n%s\n", net_stream);
    ...
    display(app_stream_size, (int)strlen(trans_stream), (int)strlen(net_stream), total_packets, total_frames);
    printf("====================================================================\n");
