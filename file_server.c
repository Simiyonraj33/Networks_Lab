...
// Helper function to handle sending a file to the client (Retrieve/Download)
void handle_retrieve(int client_socket, const char *filename) {
    ...
    // Wait briefly for client ready ACK
    usleep(10000);
...
// Helper function to handle receiving a file from the client (Store/Upload)
void handle_store(int client_socket, const char *filename) {
    ...
    while ((bytes_received = recv(client_socket, buffer, BUFFER_SIZE, 0)) > 0) {
        // Look for end-of-file signal from client
...
int main() {
    ...
    // ITERATIVE LOOP: Processes clients sequentially one at a time
    while (1) {
        printf("\n---------------------------------------------\n");
        ...
        // Command loop for active client
        while (1) {
